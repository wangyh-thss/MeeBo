...
function getMessage(json){
    ...
    for(var i = 0; i < a.length; i++){
        ...
        if(a[i].type == "receive"){
            html = html + '<div class="MeeBo_user"><div class="head_potrait"><img class="potrait_img" src="' + a[i].head + '"/></div></div><div class="MeeBo_content"><div class="user_id"><a runat="server" onclick="javascript:__doPostBack(\'go_user_btn\',\'' + a[i].userID + '\')" style="cursor:pointer">' + a[i].nickname + '</a></div><div class="text_content">' + a[i].content + '</div><div class="MeeBo_detail"><div class="Message_time">' + a[i].time + '</div></div></div></div>';
        }
        else{
