function getMeeBo(json) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (a[i].type == "Meebo") {
            html = html + '<div id="singleMeebo' + i + '" class="single_MeeBo" MeeboID="' + a[i].MeeboID + '">' + '<div class="MeeBo_user">' + '<div class="head_potrait">' + '<img class="potrait_img" src="' + a[i].head + '"/>' + '</div>' + '</div>' + '<div class="MeeBo_content">' + '<div class="user_id">' + '<a onclick="javascript:__doPostBack(\'go_user_btn\',\'' + a[i].userID + '\')" style="cursor:pointer;">' + a[i].nickname + '</a>' + '</div>';
